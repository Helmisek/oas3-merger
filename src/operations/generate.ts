...
function findComponents(
  specifications: OA3SpecificationStructure[],
): OAS3ComponentData[] {
...
export async function generateSwagger(options: SwaggerGeneratorOptions) {
  const template = YAML.parse(
    fs.readFileSync(options.configurationSpecFile, 'utf-8'),
  )
  template.paths = {}
  template.components = {}

  const generalPaths = findPaths(
    options.specifications.filter((spec) => {
