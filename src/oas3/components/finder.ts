...
export function extractOAS3ComponentData(
  specification: any,
): OAS3ComponentData {
  const componentData = Object.keys(specification).map((key) => {
    switch (key) {
      case 'schemas': {
        return {
      ...
      default: {
        throw new Error(
          `Unrecognizable key found in provided specification: ${key}`,
        )
      }
    }
  })[0]
  ...
  if (!componentData) {
    throw new Error(
      'Unable to find any component structure for provided specification!',
    )
  }
